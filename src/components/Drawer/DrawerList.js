        ...
        case "user":
          return (
            <BottomNavigationAction
              showLabel
              label="Employees"
              onClick={e => this.handleChange("user", e)}
              icon={<PersonAddIcon />}
            />
          );
        case "employee":
          return (
            <BottomNavigationAction
              showLabel
              label="Employees"
              onClick={e => this.handleChange("employee", e)}
              icon={<PersonAddIcon />}
            />
          );
        case "saas":
          return (
            <BottomNavigationAction
              showLabel
              label="Subscriptions"
              onClick={e => this.handleChange("saas", e)}
              icon={<CloudIcon />}
            />
          );
        case "software":
          return (
            <BottomNavigationAction
              showLabel
              label="Software"
              onClick={e => this.handleChange("software", e)}
              icon={<CloudOffIcon />}
            />
          );
        case "saap":
          return (
            <BottomNavigationAction
              showLabel
              label="Software"
              onClick={e => this.handleChange("saap", e)}
              icon={<CloudOffIcon />}
            />
          );
        case "hardware":
          return (
            <BottomNavigationAction
              showLabel
              label="Hardware"
              onClick={e => this.handleChange("hardware", e)}
              icon={<ComputerIcon />}
            />
          );
        default:
          ...
          {!this.state.open && (
            <BottomNavigation value={this.state.activeList} showLabels>
              <FormContext.Consumer>
                {context => this.renderNavActions(context.state)}
              </FormContext.Consumer>
            </BottomNavigation>
          )}
        </Paper>
          ...
          <RelativeContext.Provider value={{ toggle: this.toggleDrawer }}>
            <Paper elevation={4} style={styles.topNav}>
              ...
              <Grid style={styles.header} onClick={this.toggleDrawer}>
                <ExpandMoreIcon
                  style={{ fill: "white", width: "1.5em", height: "1.5em" }}
                />
              </Grid>
            </Paper>
            ...
            {({ loading, error, data, refetch }) => {
              ...
              return (
                <List style={styles.list}>
                  {Object.values(data)[0].map(record => (
                    <DrawerListItem
                      data={record}
